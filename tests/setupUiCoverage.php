...
foreach(new \DirectoryIterator('.') as $file) {
    ...
    elseif($file->isDir()) {
        foreach(new \DirectoryIterator($file->getFilename()) as $subdir) {
            ...
            insertCCCode($file->getFilename().'/'.$subdir->getFilename(), true);
        }
...
function insertCCCode(string $filename, bool $insubdir = false) {
...
$coverage->filter()->addDirectoryToWhitelist(\''.$insubdir ? '../' : ''.'src/View\');
